...
const CONTRACTS_INDEXED_LAST = new Set(['usdc_hbar_pool']);
...
export class SeFiIndexer {
  ...
  orderContractsForIndexing(contracts) {
    ...
    for (const contract of contracts) {
      ...
      if (CONTRACTS_INDEXED_LAST.has(contractName)) {
        deferredContracts.push(contract);
      } else {
        regularContracts.push(contract);
      }
    }

    return [...regularContracts, ...deferredContracts];
  }
