...
export class SeFiAgentService {
  ...
  validateCubeQuery(cubeQuery, context) {
    ...
    const usedMembers = [
      ...getCubeMembersFromQuery(cubeQuery, 'measures'),
      ...getCubeMembersFromQuery(cubeQuery, 'dimensions'),
      ...getCubeMembersFromQuery(cubeQuery, 'segments'),
      ...extractFilterMembers(cubeQuery.filters),
      ...(Array.isArray(cubeQuery.timeDimensions)
        ? cubeQuery.timeDimensions
            .filter((item) => item && typeof item === 'object' && typeof item.dimension === 'string')
            .map((item) => item.dimension)
        : []),
      ...(cubeQuery.order && typeof cubeQuery.order === 'object' ? Object.keys(cubeQuery.order) : []),
    ];
  ...
  async executePlan(planInput, requestOptions = {}) {
    ...
    if (plan.mode === 'cube_query') {
      ...
      const response = await this.fetchImpl(`${this.config.cubeApiUrl}/load`, {
        ...
        body: JSON.stringify({ query: plan.cube_query }),
      });
      const payload = await response.json();
      if (!response.ok) {
        throw createAgentError(
