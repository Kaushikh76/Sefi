...
export class DerivedPipelineService {
  ...
  queryContractLogBatch(cursorBefore, { limit, reconcile = false, reconcileWindowRows = 250000, eventNames = null } = {}) {
  ...
  async runBuiltinClmmPoolSnapshots(pipeline, { cursorBefore, limit, preview, reconcile }) {
    const batch = this.queryContractLogBatch(cursorBefore, {
      limit,
      reconcile,
    ...
    return {
      ...
      details: {
        ...
        processed_rows: batch.rows.length,
  ...
  async runBuiltinVaultActionsDecoded(pipeline, { cursorBefore, limit, preview, reconcile }) {
    ...
    for (const log of batch.rows) {
      if (!isVaultLike(log) && !isPoolLike(log)) continue;
      ...
      rows.push({
        ...
        vault_address: poolAddress,
        strategy_address: normalizeTopicAddress(log.topic2),
        pool_address: normalizeTopicAddress(log.topic3) || poolAddress,
  ...
  recomputeVaultState(vaultAddress, bonzoPriceMap) {
    ...
    return {
      ...
      vault_name: latestPosition.vault_address,
  ...
  async runBuiltinVaultStrategyState(pipeline, { cursorBefore, limit, preview, reconcile }) {
    ...
    for (const log of batch.rows) {
      if (!isVaultLike(log) && !isPoolLike(log)) continue;
      const vaultAddress = normalizeAddress(log.evm_address || log.contract_id);
      if (vaultAddress) {
        affectedVaults.add(vaultAddress);
      }
      const ownerAddress = normalizeTopicAddress(log.topic2) || normalizeTopicAddress(log.topic1);
      if (ownerAddress) {
        affectedVaults.add(ownerAddress);
      }
    }
